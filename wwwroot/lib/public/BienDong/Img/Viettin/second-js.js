...
$("#point-code").focusout(function () {
    let money = $(this).val();
    if (money != '') {
        $('#point-code-view-content').text(money.toString().replace(/\B(?=(\d{3})+(?!\d))/g, ",") + " VND");
...
$("#beneficiary-person").focusout(function(){
    ...
    if(beneficiary_person != ''){
        $('#beneficiary-person-view-content').html(`${beneficiary_person} <span class="def">Mặc định</span>`);
    }
});
