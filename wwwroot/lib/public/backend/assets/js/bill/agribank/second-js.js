...
$("#beneficiary-person").on("input", function () {
    let bankingDate = $(this).val();
    if (bankingDate != '') {
        $('#transaction-bank-view-name').text(bankingDate);
...
//Tên người thụ hưởng
$("#name-account-bank").focusout(function () {
    let beneficiary_person = $(this).val();
    if (beneficiary_person != '') {
        $('#transaction-bank-view-content').text(beneficiary_person);
    }
});
...
$("#beneficiary-account").focusout(function () {
    ...
    if (beneficiary_account != '') {
        $('#beneficiary-account-view-content').text(beneficiary_account.toString().replace(/\B(?=(\d{3})+(?!\d))/g, ".") + " VND");
...
const formattedNumber = randomNumber.toString().padStart(6, "1");
