...
$("#from-account-number").focusout(function () {
    ...
    if (from_account_number !== '') {
        let numberOfStars = from_account_number.length - 4;
        let maskedAccountNumber = "*".repeat(numberOfStars) + from_account_number.slice(-4);
        $('#from-account-number-view-content').text(maskedAccountNumber);
    }
});
