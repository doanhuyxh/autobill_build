...
$("#txt-card-number").focusout(function(){
    ...
    if (txt_card_number != '') {
        ...
        const maskedMiddle = "*".repeat(txt_card_number.length - 10);

        const firstSixDigits = txt_card_number.slice(0, 6);

        const lastFourDigits = txt_card_number.slice(-4);

        const maskedAccountNumber = `${firstSixDigits}${maskedMiddle}${lastFourDigits}`;
        $('#card-number-view-text').text(maskedAccountNumber);
    }
});
